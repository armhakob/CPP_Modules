...
void BitcoinExchange::datainit(std::string data)
{
...
bool BitcoinExchange::isvalidDate(std::string data)
{
	if (data.size() != 10)
		return false;
	if (data[4] != '-' ||  data[7] != '-')
...
bool BitcoinExchange::isvalidVal(std::string value)
{
	if (value[0] != ' ')
		return false;
	int dot = 0;
...
void BitcoinExchange::ratePrint(std::string data, float value)
{
	map_it it = data_map.lower_bound(data);
	if (it != data_map.begin() && it->first != data)
		--it;
	if (it == data_map.end())
		--it;
...
void BitcoinExchange::exchangePrint(std::string input)
{
	...
	while(std::getline(file,line))
	{
		...
		if (delim == std::string::npos || !isvalidDate(data))
		{
		...
		if (delim + 1 >= line.size() || !isvalidVal(line.substr(delim + 1)))
		{
			std::cerr << "Error: bad value => " << line.substr(delim + 1) <<std::endl;
			continue;
		}
		ratePrint(data, value);
	}
}